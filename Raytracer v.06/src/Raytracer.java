...
public class Raytracer {
    public static void main(String[] args) {
        ...
        scene.addLight(new PointLight(
                ...
                Color.WHITE,
                100.0   // higher value because attenuation divides by distance^2
        ));
        ...
        // add an OBJ
        List<Triangle> objTriangles1 =  OBJ.read("Temple.obj", Color.RED, 1);
        for (Triangle t : objTriangles1) { // iterates over ALL triangles of the OBJ
            ...
            if (t.hasVertexNormals()) {
                scene.addObject(new Triangle(v0, v1, v2,
                        t.getNormal0(), t.getNormal1(), t.getNormal2(), Color.RED));
            } else {
                scene.addObject(new Triangle(v0, v1, v2, Color.RED)); // to construct a triangle of the object, it needs three vertices (and color)
            }
        }
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (intersect != null) {
                    ...
                    Color shadedColor = applyShading(intersect, scene.getLights(), cameraReal);
    ...
    static Color applyShading(Intersection intersect, List<Light> lights, Camera camera) {
