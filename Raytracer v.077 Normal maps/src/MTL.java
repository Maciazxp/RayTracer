...
public class MTL {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (parts[0].equals("newmtl")) {
                    ...
                    // Diffuse texture — may have spaces in path, so take everything after keyword
                    String path = line.substring("map_Kd".length()).trim();
                    current.loadDiffuseTexture(path);

                } else if (parts[0].equals("map_Bump") && current != null) {
                    // Normal map — Blender exports it as: map_Bump -bm 1.000000 <path>
                    // The path is always the LAST token, after any optional flags like -bm
    ...
    private static String extractTexturePath(String line, String keyword) {
        ...
        while (i < tokens.length - 1 && tokens[i].startsWith("-")) {
            i += 2; // skip the flag and its value (e.g. "-bm" "1.000000")
        }
