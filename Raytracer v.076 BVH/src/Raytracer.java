...
public class Raytracer {
    public static void main(String[] args) {
        ...
        // create and directly add

        // METHOD 1 (only adding an object with mtl) with scale (cant move)
        /*
        List<Triangle> objTriangles = OBJ.read("triforce.obj", "triforce.mtl", 3.0);
        scene.addMesh(objTriangles);

        /*
        //METHOD 2 (adding an object with solid color) with scale (cant move)
        scene.addMesh(OBJ.read("sword.obj", Color.RED, 0.15));
        //METHOD 3 (simply way to add an object with mtl) also scaled
        scene.addMesh(OBJ.read("triforce.obj", "triforce.mtl", 3.0));

        */

        /*
        //METHOD 4 (most large and complete method to add an obj from main) you can move everything (EXCEPT COLOR)
        List<Triangle> objTriangles = OBJ.read("triforce.obj", "triforce.mtl", 3.0);
        ...
        */
        ...
        //METHOD 5 (add an OBJ, with MTL, and you could move the POSITION OF THE OBJECT) also the scale
        List<Triangle> baseMesh = OBJ.read("triforce.obj", "triforce.mtl", 1.0);
        scene.addMesh(baseMesh, new Vector3D(0, 0, 0), 3.0);

        /*
        //METHOD 6 (add an OBJ, with solid color, and you could move the position)
        List<Triangle> baseMesh = OBJ.read("triforce.obj", Color.CYAN, 1.0);
        scene.addMesh(baseMesh, new Vector3D(0, 0, 0), 3.0);
        */
        /*
        //------------ADD TEST OBJ TEAPOTS------------------
        scene.addMesh(OBJ.read("SmallTeapot.obj", Color.RED, 0.30));
        List<Triangle> baseMesh = OBJ.read("SmallTeapot.obj", Color.CYAN, 0.3);
        scene.addMesh(baseMesh, new Vector3D(0.3, 0, 0.8), 0.8);
           */
